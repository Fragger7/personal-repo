...
* **Committed Tab Cloud Sync Crash**: Fixed a critical NPE crash by refactoring `CommittedRecord` parsing logic to utilize safe nullable getters, preventing the app from crashing when encountering null or missing JSON fields (e.g., `notes` or `dateAdded`) from the Python cloud database.
...
* **Xtream Master Level Channel Query**: Implemented a "Query All Active" master button in the Xtream header. This loops through active nodes concurrently, fetching their Live Channel and VOD counts without requiring users to dive into each row individually. (Note: Not implemented on Stalker tab due to Ministra Framework firewall constraints detailed in the architecture).
* **Deep-Dive Category Navigation Crash**: Resolved a core UI crash caused by rapid state changes when navigating backwards from Channel lists to Categories in the Xtream Details view. Re-engineered the LazyColumn generation to use immutable snapshot references of state arrays, preventing concurrent modification/null exceptions during recomposition.
